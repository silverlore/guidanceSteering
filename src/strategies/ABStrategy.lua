...
function ABStrategy:draw(data, guidanceSteeringIsActive, autoInvertOffset)
	...
	if not drawDirectionLine then
		...
		local lineX = x - data.headlineArcDistance * lineDirX * direction - headlineLineOffset * lineDirZ * direction + data.width * lineDirZ * beta
        local lineZ = z - data.headlineArcDistance * lineDirZ * direction + headlineLineOffset * lineDirX * direction - data.width * lineDirX * beta
